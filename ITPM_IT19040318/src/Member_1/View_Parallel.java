...
public class View_Parallel {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					View_Parallel window = new View_Parallel();
	...
	private void initialize() {
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					Connection con = DBConnect.connect();
					String query="Delete from parallel where ID='"+textField.getText()+"'";
					PreparedStatement pst=con.prepareStatement(query);
					pst.execute();
		...
		JLabel lblManageSession = new JLabel("Manage Session - Parallel");
		lblManageSession.setForeground(Color.BLACK);
		lblManageSession.setFont(new Font("Tahoma", Font.BOLD, 25));
		lblManageSession.setBackground(Color.MAGENTA);
		lblManageSession.setBounds(38, 94, 436, 48);
		frame.getContentPane().add(lblManageSession);
